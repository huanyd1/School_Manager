<?php

include "../../../../config/config.php";
session_start();

$sql = "SELECT * FROM `quanlytruonghoc`.`sinhvien` WHERE `idSinhvien` = '$_GET[idSinhvien]' ";

$result = mysqli_query($conn, $sql);

$row = mysqli_fetch_array($result);

$sql_idLop = "SELECT * FROM `quanlytruonghoc`.`lop`";

$query_idLop = mysqli_query($conn, $sql_idLop);


if (isset($_POST["process"])) {

    $tenSinhvien = mysqli_escape_string($conn, $_POST["tenSinhvien"]);

    $idSinhvien = $_POST['idSinhvien'];

    $tenSinhvien = $_POST["tenSinhvien"];

    $gioiTinh = $_POST["gioiTinh"];

    $idLop = $_POST['idLop'];

    $img = $_FILES['imgUpload']['name'];



    if ($img != null) {

        $path = "imgUpload/";

        $tmp_name = $_FILES['imgUpload']['tmp_name'];

        $img = $_FILES['imgUpload']['name'];

        move_uploaded_file($tmp_name, $path . $img);

        $sql = "UPDATE `quanlytruonghoc`.`sinhvien` SET `imgSinhvien` = '$img' where `idSinhvien` ='$_GET[idSinhvien]'";

        mysqli_query($conn, $sql);

        header('location:sinhvien.php?page_layout=danhsach');
    }



    $sql = "UPDATE `quanlytruonghoc`.`sinhvien` SET `tenSinhvien` = '$tenSinhvien', `gioiTinh` = '$gioiTinh', `idLop` = '$idLop' where `idSinhvien` = '$_GET[idSinhvien]' ";

    mysqli_query($conn, $sql);

    header('location:sinhvien.php?page_layout=danhsach');
}



?>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <title>DANH SÁCH KHOA</title>
    <link rel="stylesheet" href="../../../css/add.css">
    <link rel="stylesheet" href="../../../css/styles.css">
</head>

<body>
    <div class="header">
        <div class="bao-logo">
            <img class="logo" src="../../../image/logo.png" alt="">
        </div>
        <div class="user">
            <p><?php echo @$_SESSION["user"] ?></p>
        </div>
        <div class="out">
            <a href="">Đăng xuất</a>
        </div>
    </div>
    <!-- <div class="banner">
        <img src="../../image/img.jpg" alt="">
    </div> -->
    <div class="container">
        <div class="content">
            <div class="left">
                <h2><a href="">Trang chủ</a></h2>
                <ul class="ul-left">
                    <li class="li-left">
                        <a class="a-left" href="../isAdmin/list.php">USER</a>
                    </li>
                    <li class="li-left">
                        <a class="a-left" href="../Khoa/khoa.php">KHOA</a>
                    </li>
                    <li class="li-left">
                        <a class="a-left" href="../Bomon/bomon.php">BỘ MÔN</a>
                    </li>
                    <li class="li-left">
                        <a class="a-left" href="../Lop/lop.php">LỚP</a>
                    </li>
                    <li class="li-left">
                        <a class="a-left" href="../Giangvien/giangvien.php">GIẢNG VIÊN</a>
                    </li>
                    <li class="li-left active">
                        <a class="a-left" href="../sinhvien.php">SINH VIÊN</a>
                    </li>
                    <li class="li-left">
                        <a class="a-left" href="../Monhoc/monhoc.php">MÔN HỌC</a>
                    </li>
                    <li class="li-left">
                        <a class="a-left" href="../Diemthi/diemthi.php">ĐIỂM</a>
                    </li>
                </ul>
            </div>
            <div class="right">
                <div class='title'>
                    <h2>SỬA SINH VIÊN</h2>
                </div>
                <form class="form" action="" method="post" enctype="multipart/form-data">
                    <div class="form-input">
                        <div class="text">
                            <p>Mã Sinh Viên</p>
                        </div>
                        <div class="input-right">
                            <input type="text" placeholder="Mã sinh viên" name="idSinhvien" value="<?php echo $row['idSinhvien']; ?>" name="idSinhvien">
                        </div>
                    </div>
                    <div class="form-input">
                        <div class="text">
                            <p>Tên Sinh Viên</p>
                        </div>
                        <div class="input-right">
                            <input type="text" placeholder="Tên sinh viên" name="tenSinhvien" value="<?php echo $row['tenSinhvien']; ?>" name="tenSinhvien">
                        </div>
                    </div>
                    <div class="form-input">
                        <div class="text">
                            <p>Ảnh Sinh Viên</p>
                        </div>
                        <div class="input-right">
                            <img src="imgUpload/<?php echo $row['imgSinhvien']; ?>" style="max-width: 100px;">
                        </div>
                    </div>
                    <div class="form-input">
                        <div class="text">
                            <p></p>
                        </div>
                        <div class="input-right">
                            <button style="display:block;width:120px; height:30px;" onclick="document.getElementById('getFile').click()">Thay ảnh mới</button>
                            <input type='file' id="getFile" name="imgUpload" style="display:none"> </div>
                    </div>
                    <div class="form-input">
                        <div class="text">
                            <p>Giới Tính</p>
                        </div>
                        <div>
                            <select name="gioiTinh">
                                <option <?php if ($row['gioiTinh'] == 'Nam') {
                                            echo "selected";
                                        } ?>>Nam</option>
                                <option <?php if ($row['gioiTinh'] == 'Nữ') {
                                            echo "selected";
                                        } ?>>Nữ</option>
                            </select>
                        </div>
                    </div>
                    <div class="form-input">
                        <div class="text">
                            <p>Ngày Sinh</p>
                        </div>
                        <div>
                            <input type="date" name="ngaySinh" value="<?php echo $row['ngaySinh']; ?>">
                        </div>
                    </div>
                    <div class="form-input">
                        <div class="text">
                            <p>Sinh viên của Lớp</p>
                        </div>
                        <div class="input-right">
                            <select name="idLop">
                                <?php
                                while ($row_idLop = mysqli_fetch_assoc($query_idLop)) { ?>
                                    <option value="<?php echo $row_idLop['idLop']; ?>" <?php if ($row['idLop'] == $row_idLop['idLop']) {
                                                                                            echo ("selected");
                                                                                        } ?>>
                                        <?php echo $row_idLop['tenLop']; ?>
                                    </option>
                                <?php } ?></select>
                        </div>
                    </div>
                    <div class="sb">
                        <input type="submit" name="process" value="Update">
                    </div>

                </form>
            </div>
        </div>
</body>

</html>